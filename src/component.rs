...
use std::{fmt::Debug, ops::{Sub, Add}, usize};
...
impl Component for TfAdd {
    ...
    fn make_expression<'a>(
        ...
        // let mut file_add = std::fs::File::create("add.txt").expect("create failed");
        // file_add.write_all(operands[0].vecs.to_string().as_bytes()).expect("lib write failed");
        let mut result = Vecs::new(operands[0].dims);
        //具体的size存储在Vecs.dims中，是Int变量，对于add就暂时不管了，反正用不上
        for i in 0 .. DIMS[0] {
            for j in 0 .. DIMS[1] {
